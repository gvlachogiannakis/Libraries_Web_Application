...
function checkPassword() {
    ...
    if (password2 === '') {
        ...
        document.getElementById("password_message_repeat").innerHTML = "<style> #password_message_repeat {color: red;}</style>Empty Repeated Password ";
    } else if (password1 !== password2) {
        // If Not same return False.
        document.getElementById("password_message_repeat").innerHTML = "<style> #password_message_repeat {color: red;}</style>Different Passwords";
    } else {
        // If same return True.
        document.getElementById("password_message_repeat").innerHTML = "<style> #password_message_repeat {color: green;}</style>Password Matches";
    }
}
...
function password_strength() {
    ...
    if (password1.includes("helmepa") || password1.includes("uoc") || password1.includes("tuc")) {
        document.getElementById("password_message").innerHTML = "<style> #password_message {color: red;}</style>Illegal Input Password<br>You can't use the Words:<br>helmepa, uoc , tuc.";
    } else {
...
function checkUsers() {
    ...
    if (finalType === "student") {
        ...
        if (finalEducation === "UOC" && email.slice(email.length - 6) === "uoc.gr") {
            document.getElementById("student_message").innerHTML = "<style> #student_message {color: green;}</style>Correct Mail";
        } else if (finalEducation === "HELMEPA" && email.slice(email.length - 10) === "helmepa.gr") {
            document.getElementById("student_message").innerHTML = "<style> #student_message {color: green;}</style>Correct Mail";
        } else if (finalEducation === "TUC" && email.slice(email.length - 6) === "tuc.gr") {
            document.getElementById("student_message").innerHTML = "<style> #student_message {color: green;}</style>Correct Mail";
        } else {
            document.getElementById("student_message").innerHTML = "<style> #student_message {color: red;}</style>Wrong Mail";
        }
